...
public class SafariApplication {
	...
	@Bean
	public CommandLineRunner demo(CustomerRepo custrepo, EmployeeRepo emprepo, TripRepo triprepo,
			TripInstanceRepo tripinrepo, ReservationRepo resrepo, SafUserRepo userpo) {
		...
		return (args) -> {
			...
			triprepo.save(new Trip("SnowVillage", "Lainiontie 2", "Lainio SnowVillage", "Helene", "Curie", "040658932",
					"Trip to SnowVillage to see Santa", 30.50, 17.40, false));
			triprepo.save(new Trip("Elf village", "Könkääntie 42", "Tonttula", "Perttu", "Palovaara", "023565146",
					"Come to see Santa and experience winter with elves", 34.80, 25.30, false));
			triprepo.save(new Trip("Snowmobiles at night", "Levinraitti 14", "SM routes", "Sakari", "Moottori",
					"35413205", "Come drive a snowmobile at night, with warm juice and gingerbread cookies", 140.40, 0,
					true));
			...
			resrepo.save(new Reservation(custrepo.getByPartyName("Miller"),
					tripinrepo.getByEmployeeLastName("Villiers"), 2, 2));
			resrepo.save(new Reservation(custrepo.getByPartyName("Corey"), tripinrepo.getByEmployeeLastName("Loponen"),
					1, 2));

			userpo.save(new SafariUser("user", "$2a$05$IOA6zU6POEMZ1uiOw/8ckurS58.wgRuss95WUUxSrSw6gzO4coApm", "USER"));
			userpo.save(
					new SafariUser("admin", "$2a$05$FSsAAxlysvCpWkp5RBHXGeSvRo1FpNp3O1DOpXW3hoPublP9sPHgW", "ADMIN"));
